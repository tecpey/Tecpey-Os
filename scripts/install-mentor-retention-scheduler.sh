...
sed \
  ...
  "$TEMPLATE_DIR/tecpey-mentor-retention.service.in" > "$TMP_DIR/tecpey-mentor-retention.service"
cp "$TEMPLATE_DIR/tecpey-mentor-retention.timer" "$TMP_DIR/"

if grep -Eq '@@[A-Z_]+@@' "$TMP_DIR/tecpey-mentor-retention.service"; then
  fail "systemd_template_placeholder_unresolved"
fi

systemd-analyze verify \
  "$TMP_DIR/tecpey-mentor-retention.service" \
  "$TMP_DIR/tecpey-mentor-retention.timer" >/dev/null
...
install -m 0644 "$TMP_DIR/tecpey-mentor-retention.service" "$SYSTEMD_DIR/"
install -m 0644 "$TMP_DIR/tecpey-mentor-retention.timer" "$SYSTEMD_DIR/"

systemctl daemon-reload
systemctl enable --now tecpey-mentor-retention.timer
systemctl is-enabled --quiet tecpey-mentor-retention.timer
systemctl is-active --quiet tecpey-mentor-retention.timer
bash "$APP_DIR/scripts/check-systemd-timer-scheduled.sh" tecpey-mentor-retention.timer

printf 'installed=1\n'
printf 'mentor_retention_timer=active\n'
