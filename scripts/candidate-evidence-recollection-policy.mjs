...
function exactSet(findings, label, actual, expected) {
  ...
  if (new Set(actual).size !== actual.length) findings.push(`${label}: duplicate entries are forbidden`);
...
function requireEqual(findings, label, actual, expected) {
  if (actual !== expected) findings.push(`${label}: expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`);
}

function requireSha(findings, label, value) {
  if (typeof value !== "string" || !SHA.test(value)) findings.push(`${label}: expected exact lowercase 40-character SHA`);
}
...
export function candidateEvidenceRecollectionFindings({ request, promotionState }) {
  ...
  requireEqual(findings, "request.evidenceClass", request?.evidenceClass, "controlled-launch-candidate-evidence-recollection-request");
  requireEqual(findings, "request.issue", request?.issue, 515);
  requireEqual(findings, "request.decision", request?.decision, "NO_GO_PENDING_NOG_03_04_06_RECOLLECTION");
  ...
  requireEqual(findings, "promotionState.evidenceClass", promotionState?.evidenceClass, "controlled-launch-candidate-promotion-state");
  ...
  requireEqual(findings, "promotionState.protectedExecutionAllowed", promotionState?.protectedExecutionAllowed, false);
  requireEqual(findings, "promotionState.requiredAcceptanceSchemaVersion", promotionState?.requiredAcceptanceSchemaVersion, 2);
  requireSha(findings, "promotionState.currentAcceptedCandidateSha", promotionState?.currentAcceptedCandidateSha);
  requireSha(findings, "promotionState.proposedCandidate.sha", promotionState?.proposedCandidate?.sha);
  requireEqual(findings, "promotionState.proposedCandidate.sourceBranch", promotionState?.proposedCandidate?.sourceBranch, "main");
  ...
  requireEqual(findings, "request.selectedSha", request?.selectedSha, promotionState?.proposedCandidate?.sha);
  requireEqual(findings, "request.sourcePullRequest", request?.sourcePullRequest, promotionState?.proposedCandidate?.sourcePullRequest);

  exactSet(findings, "request.requiredEvidence.NOG-04.requiredWorkflows", request?.requiredEvidence?.["NOG-04"]?.requiredWorkflows, REQUIRED_WORKFLOWS);
  exactSet(findings, "request.stillOpenBlockers", request?.stillOpenBlockers, REQUIRED_OPEN);
  exactSet(findings, "promotionState.stillOpenBlockers", promotionState?.stillOpenBlockers, REQUIRED_OPEN);
  exactSet(findings, "request.launchDisabledBoundaries", request?.launchDisabledBoundaries, REQUIRED_BOUNDARIES);
  exactSet(findings, "promotionState.launchDisabledBoundaries", promotionState?.launchDisabledBoundaries, REQUIRED_BOUNDARIES);
  exactSet(findings, "promotionState.staleAcceptedEvidence", promotionState?.staleAcceptedEvidence?.map((entry) => entry?.id), REQUIRED_STALE);

  for (const blocker of ["NOG-03", "NOG-04", "NOG-06"]) {
    requireEqual(findings, `request.requiredEvidence.${blocker}.status`, request?.requiredEvidence?.[blocker]?.status, "pending_recollection");
    requireEqual(findings, `request.requiredEvidence.${blocker}.requireExactSelectedSha`, request?.requiredEvidence?.[blocker]?.requireExactSelectedSha, true);
  }

  requireEqual(findings, "NOG-03.workflow", request?.requiredEvidence?.["NOG-03"]?.workflow, "Container Supply Chain");
  requireEqual(findings, "NOG-06.workflow", request?.requiredEvidence?.["NOG-06"]?.workflow, "Container Supply Chain");
  requireEqual(findings, "NOG-06.job", request?.requiredEvidence?.["NOG-06"]?.job, "Ephemeral staging rollback and volume restore");
  ...
  for (const token of ["raw secrets", "database URLs", "host IPs", "customer data", "raw logs", "private keys"]) {
    requireText(findings, "request.privacyBoundary", request?.privacyBoundary, token);
  }
