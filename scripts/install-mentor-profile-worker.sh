...
WORKER_TEMPLATE="${TEMPLATE_DIR}/tecpey-mentor-profile-worker.service.in"
HEALTH_TEMPLATE="${TEMPLATE_DIR}/tecpey-mentor-profile-health.service.in"
HEALTH_TIMER="${TEMPLATE_DIR}/tecpey-mentor-profile-health.timer"
ALERT_TEMPLATE="${TEMPLATE_DIR}/tecpey-ops-alert-delivery.service.in"
ALERT_TIMER="${TEMPLATE_DIR}/tecpey-ops-alert-delivery.timer"
...
for bundle in   run-mentor-profile-worker.cjs   check-mentor-profile-health.cjs   deliver-operational-alerts.cjs   check-operational-alert-delivery-env.cjs
do
  [[ -f "$APP_DIR/dist/$bundle" && ! -L "$APP_DIR/dist/$bundle" ]]     || fail "required_runtime_bundle_missing"
done

for source in   "$WORKER_TEMPLATE"   "$HEALTH_TEMPLATE"   "$HEALTH_TIMER"   "$ALERT_TEMPLATE"   "$ALERT_TIMER"
do
  [[ -f "$source" && ! -L "$source" ]] || fail "systemd_source_missing"
done
...
render_service() {
  ...
  sed     -e "s|@@RUN_USER@@|$(escape_sed "$RUN_USER")|g"     -e "s|@@RUN_GROUP@@|$(escape_sed "$RUN_GROUP")|g"     -e "s|@@APP_DIR@@|$(escape_sed "$APP_DIR")|g"     -e "s|@@ENV_FILE@@|$(escape_sed "$ENV_FILE")|g"     -e "s|@@STATE_DIR@@|$(escape_sed "$STATE_DIR")|g"     -e "s|@@NPM_BIN@@|$(escape_sed "$NPM_BIN")|g"     "$source" > "$destination"
...
systemd-analyze verify   "$TMP_DIR/tecpey-mentor-profile-worker.service"   "$TMP_DIR/tecpey-mentor-profile-health.service"   "$TMP_DIR/tecpey-mentor-profile-health.timer"   "$TMP_DIR/tecpey-ops-alert-delivery.service"   "$TMP_DIR/tecpey-ops-alert-delivery.timer" >/dev/null
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  printf 'durable_signal_delivery=verified\n'
  exit 0
fi
...
printf 'operational_alert_delivery_timer=active\n'
printf 'state_directory=%s\n' "$STATE_DIR"
