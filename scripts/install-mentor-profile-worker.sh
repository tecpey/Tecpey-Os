...
for template_file in   "$WORKER_TEMPLATE"   "$HEALTH_TEMPLATE"   "$HEALTH_TIMER"   "$ALERT_TEMPLATE"   "$ALERT_TIMER"
do
  [[ -f "$template_file" && ! -L "$template_file" ]] || fail "mentor_operational_unit_template_missing"
done
...
render_service() {
  ...
  sed     -e "s|@@RUN_USER@@|$(escape_sed "$RUN_USER")|g"     -e "s|@@RUN_GROUP@@|$(escape_sed "$RUN_GROUP")|g"     -e "s|@@APP_DIR@@|$(escape_sed "$APP_DIR")|g"     -e "s|@@ENV_FILE@@|$(escape_sed "$ENV_FILE")|g"     -e "s|@@STATE_DIR@@|$(escape_sed "$STATE_DIR")|g"     -e "s|@@NPM_BIN@@|$(escape_sed "$NPM_BIN")|g"     "$template" > "$output"
...
printf 'operational_delivery_timer=active\n'
printf 'state_directory=%s\n' "$STATE_DIR"
