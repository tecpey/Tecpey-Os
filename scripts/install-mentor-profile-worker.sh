...
[[ -f "$ALERT_TEMPLATE" && ! -L "$ALERT_TEMPLATE" ]] || fail "operational_alert_service_template_missing"
[[ -f "$ALERT_TIMER" && ! -L "$ALERT_TIMER" ]] || fail "operational_alert_timer_missing"
...
grep -Eq '^TECPEY_OPS_ALERT_WEBHOOK_URL=https://[^[:space:]]+
...
printf 'state_directory=%s\n' "$STATE_DIR"
 "$ENV_FILE" \
  || fail "ops_alert_https_webhook_missing"
if grep -Eq '^DATABASE_URL=.*(CHANGE_ME|example\.invalid)' "$ENV_FILE"; then
  fail "database_url_placeholder"
fi
if grep -Eq '^TECPEY_OPS_ALERT_WEBHOOK_URL=.*(CHANGE_ME|example\.invalid|localhost|127\.0\.0\.1)' "$ENV_FILE"; then
  fail "ops_alert_webhook_placeholder"
fi
id "$RUN_USER" >/dev/null 2>&1 || fail "runtime_user_missing"
getent group "$RUN_GROUP" >/dev/null 2>&1 || fail "runtime_group_missing"
command -v systemd-analyze >/dev/null 2>&1 || fail "systemd_analyze_missing"
