...
requireText("migration", "notification_domain_outbox_identity_no_update", "committed event identity and payload must be immutable");
requireText("migration", "notification_domain_outbox_no_delete", "committed domain events must not be deleted");
...
requireText("outbox", "RETURNING o.id, o.attempt_count, o.max_attempts", "claims must contain lease coordinates only");
rejectText("outbox", "RETURNING o.id, o.attempt_count, o.max_attempts, o.tenant_id", "claims must not materialize event payload or recipient data");
...
requireText("processing", "hashNotificationDomainEvent(occurrenceEvent)", "processing must verify stored event integrity");
requireText("processing", "notification_domain_event_fingerprint_mismatch", "fingerprint mismatches must fail closed");
requireText("processing", "produceDomainNotification(client, effectiveEvent)", "authoritative processing must delegate only the verified effective event");
...
requireText("processing", "eventFingerprint", "attempt evidence must preserve verified fingerprint");
rejectText("processing", "claim.event", "in-memory claim content must have no processing authority");
...
requireText("workerPolicy", "notification_domain_event_fingerprint_mismatch", "event integrity failures must be terminal");
...
for (const directory of [path.join(root, "src"), path.join(root, "scripts")]) {
  for (const absolute of await walk(directory)) {
    ...
    if (relative === path.normalize(files.guard)) continue;
    if ((await readFile(absolute, "utf8")).includes("processClaimedNotificationDomainEvent")) {
      failures.push(`${relative}: deprecated in-memory claim processor is forbidden`);
    }
  }
}
...
console.log(
  "Notification domain outbox authority check passed: atomic enqueue, immutable canonical fingerprints, payload-free claims, authoritative reload, leased bounded concurrency, retry/DLQ, locale-safe rendering and Academy integration are enforced.",
);
