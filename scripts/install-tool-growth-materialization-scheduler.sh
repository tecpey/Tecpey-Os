...
sed \
  ...
  "$TEMPLATE_DIR/tecpey-tool-growth-materialization.service.in" > "$TMP_DIR/tecpey-tool-growth-materialization.service"
cp "$TEMPLATE_DIR/tecpey-tool-growth-materialization.timer" "$TMP_DIR/"
if grep -Eq '@@[A-Z_]+@@' "$TMP_DIR/tecpey-tool-growth-materialization.service"; then
  fail "systemd_template_placeholder_unresolved"
fi
systemd-analyze verify "$TMP_DIR/tecpey-tool-growth-materialization.service" "$TMP_DIR/tecpey-tool-growth-materialization.timer" >/dev/null

if [[ "$DRY_RUN" == "1" ]]; then
  printf 'dry_run=1\n'
  printf 'scheduler=tool-growth-materialization\n'
  printf 'app_dir=%s\n' "$APP_DIR"
...
install -m 0644 "$TMP_DIR/tecpey-tool-growth-materialization.service" "$SYSTEMD_DIR/"
install -m 0644 "$TMP_DIR/tecpey-tool-growth-materialization.timer" "$SYSTEMD_DIR/"
systemctl daemon-reload
systemctl enable --now tecpey-tool-growth-materialization.timer
systemctl is-enabled --quiet tecpey-tool-growth-materialization.timer
systemctl is-active --quiet tecpey-tool-growth-materialization.timer
printf 'installed=1\n'
printf 'tool_growth_materialization_timer=active\n'
