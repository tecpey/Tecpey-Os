...
const manifestPath = path.resolve(root, "api-security-manifest.generated.json");
const baselinePath = path.resolve(root, "docs/security/generated/api-security-manifest.json");
const registryPath = path.resolve(root, "config/api-security-exceptions.json");
...
for (const entry of manifest.routes) {
  for (const finding of entry.findings) {
    if (!contracts[finding]) throw new Error(`Unmapped API security finding: ${finding}`);
    const operations = grouped.get(finding) ?? [];
    operations.push({ route: entry.route, method: entry.method, owner: ownerFor(entry.route) });
    grouped.set(finding, operations);
  }
}

const groups = Object.entries(contracts)
  .filter(([finding]) => grouped.has(finding))
  .map(([finding, contract]) => ({
    ...
    operations: grouped.get(finding)
  }));

await mkdir(path.dirname(baselinePath), { recursive: true });
await writeFile(baselinePath, `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
await writeFile(registryPath, `${JSON.stringify({ schemaVersion: 2, groups }, null, 2)}\n`, "utf8");
console.log(JSON.stringify({ totals: manifest.totals, groups: groups.map((group) => ({ id: group.id, count: group.operations.length })) }, null, 2));
