...
async function addSemanticEvidenceFixture(root, overrides = {}) {
  ...
  const eighthTargetPath = "scripts/production-ui-verification-authority.mjs";
  ...
  await fs.writeFile(
    path.join(root, fifthTargetPath),
    "export const contributionGovernanceGoverned = true;\n",
    "utf8",
  );
  await fs.writeFile(
    path.join(root, sixthTargetPath),
  ...
  await fs.writeFile(
    path.join(root, eighthTargetPath),
    "export const productionUiVerificationGoverned = true;\n",
    "utf8",
  );
  await git(root, "add", ".");
  ...
  const eighthContent = await fs.readFile(path.join(root, eighthTargetPath));
  const gitObjectId = (
  ...
  const fifthGitObjectId = (
    await execFileAsync("git", ["rev-parse", `HEAD:${fifthTargetPath}`], { cwd: root })
  ).stdout.trim();
  ...
  const eighthGitObjectId = (
    await execFileAsync("git", ["rev-parse", `HEAD:${eighthTargetPath}`], { cwd: root })
  ).stdout.trim();
  const declaration = {
    schemaVersion: 1,
  ...
  const fifthDeclaration = {
    schemaVersion: 1,
    evidenceId: "batch-01e-contribution-governance",
    reviewBatch: 1,
    reviewedAt: "2026-07-30",
    reviewMethod: "Complete line-addressable semantic review of a fifth governed fixture.",
    residualRisks: [
      {
        id: "B01E-RISK-001",
        severity: "P3",
        statement: "Fifth fixture review does not claim repository-wide completion.",
        owner: "audit-test",
        disposition: "tracked-debt",
      },
    ],
    files: [
      {
        path: fifthTargetPath,
        gitObjectId: fifthGitObjectId,
        sha256: createHash("sha256").update(fifthContent).digest("hex"),
        lines: 1,
        reviewedRanges: [{ startLine: 1, endLine: 1 }],
        findingDisposition: "no-confirmed-findings",
        reviewNotes: ["The fifth exported authority line was reviewed."],
        findings: [],
      },
    ],
  };
  const fifthEvidencePath = path.join(
    root,
    "docs",
    "audits",
    "evidence",
    "batch-01e-contribution-governance.json",
  );
  const sixthDeclaration = {
    schemaVersion: 1,
    evidenceId: "batch-01f-platform-core",
    reviewBatch: 1,
    reviewedAt: "2026-07-30",
    reviewMethod: "Complete line-addressable semantic review of a sixth governed fixture.",
    residualRisks: [
      {
        id: "B01F-RISK-001",
        severity: "P3",
        statement: "Sixth fixture review does not claim repository-wide completion.",
        owner: "audit-test",
        disposition: "tracked-debt",
      },
    ],
    files: [
      {
        path: sixthTargetPath,
        gitObjectId: sixthGitObjectId,
        sha256: createHash("sha256").update(sixthContent).digest("hex"),
        lines: 1,
        reviewedRanges: [{ startLine: 1, endLine: 1 }],
        findingDisposition: "no-confirmed-findings",
        reviewNotes: ["The sixth exported authority line was reviewed."],
        findings: [],
      },
    ],
  };
  const sixthEvidencePath = path.join(
    root,
    "docs",
    "audits",
    "evidence",
  ...
  const eighthDeclaration = {
    schemaVersion: 1,
    evidenceId: "batch-01h-production-ui-verification",
    reviewBatch: 1,
    reviewedAt: "2026-07-30",
    reviewMethod: "Complete line-addressable semantic review of an eighth governed fixture.",
    residualRisks: [
      {
        id: "B01H-RISK-001",
        severity: "P3",
        statement: "Eighth fixture review does not claim repository-wide completion.",
        owner: "audit-test",
        disposition: "tracked-debt",
      },
    ],
    files: [
      {
        path: eighthTargetPath,
        gitObjectId: eighthGitObjectId,
        sha256: createHash("sha256").update(eighthContent).digest("hex"),
        lines: 1,
        reviewedRanges: [{ startLine: 1, endLine: 1 }],
        findingDisposition: "no-confirmed-findings",
        reviewNotes: ["The eighth exported authority line was reviewed."],
        findings: [],
      },
    ],
  };
  const eighthEvidencePath = path.join(
    root,
    "docs",
    "audits",
    "evidence",
    "batch-01h-production-ui-verification.json",
  );
  ...
  await fs.writeFile(
    fifthEvidencePath,
    `${JSON.stringify(fifthDeclaration, null, 2)}\n`,
    "utf8",
  );
  await fs.writeFile(
    sixthEvidencePath,
  ...
  await fs.writeFile(
    eighthEvidencePath,
    `${JSON.stringify(eighthDeclaration, null, 2)}\n`,
    "utf8",
  );
  await git(root, "add", ".");
...
test("manifest inventories the exact committed tree with deterministic evidence", async (t) => {
  ...
  assert.equal(first.summary.trackedPaths, 20);
  ...
  assert.equal(first.reviewEvidenceSets.length, 8);
  assert.equal(
...
test("manifest applies line-addressable semantic evidence to the exact reviewed blob", async (t) => {
  ...
  assert.equal(manifest.reviewEvidenceSets.length, 8);
