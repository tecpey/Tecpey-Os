...
sed \
  ...
  "$TEMPLATE_DIR/tecpey-news-materialization.service.in" > "$TMP_DIR/tecpey-news-materialization.service"
cp "$TEMPLATE_DIR/tecpey-news-materialization.timer" "$TMP_DIR/"

if grep -Eq '@@[A-Z_]+@@' "$TMP_DIR/tecpey-news-materialization.service"; then
  fail "systemd_template_placeholder_unresolved"
fi

systemd-analyze verify \
  "$TMP_DIR/tecpey-news-materialization.service" \
  "$TMP_DIR/tecpey-news-materialization.timer" >/dev/null
...
install -m 0644 "$TMP_DIR/tecpey-news-materialization.service" "$SYSTEMD_DIR/"
install -m 0644 "$TMP_DIR/tecpey-news-materialization.timer" "$SYSTEMD_DIR/"
