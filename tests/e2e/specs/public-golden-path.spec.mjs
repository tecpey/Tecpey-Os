...
test.beforeEach(async ({ context, page }) => {
  ...
  page.on("console", (message) => {
    ...
    if (/(Unhandled|Hydration failed|TypeError|ReferenceError|SyntaxError)/i.test(text)) {
...
test("localized shell, primary journeys and responsive geometry are truthful", async ({ page }, testInfo) => {
...
test("theme control switches and persists without hydration drift", async ({ page }, testInfo) => {
  const contract = projectContract(testInfo);
  await openPublicPage(page, contract);
...
test("Knowledge Center is keyboard-operable on desktop and reachable on mobile", async ({ page }, testInfo) => {
  const contract = projectContract(testInfo);
  await openPublicPage(page, contract);
  ...
  if (contract.formFactor === "desktop") {
    ...
    await expect(page.getByRole("link", { name: contract.arena })).toBeVisible();
    await page.keyboard.press("Escape");
    await expect(menuTrigger).toHaveAttribute("aria-expanded", "false");
  }
});

test("public Mentor entry stays educational, modal and focus-safe", async ({ page }, testInfo) => {
  const contract = projectContract(testInfo);
  await openPublicPage(page, contract);

  const trigger = page.getByRole("button", { name: contract.mentor });
  await expect(trigger).toBeVisible();
  await trigger.click();

  const dialog = page.getByRole("dialog", { name: contract.mentorTitle });
  await expect(dialog).toBeVisible();
  await expect(dialog).toContainText(contract.locale === "en" ? "does not provide guaranteed-profit" : "وعده سود تضمینی");
...
test("content remains usable without IntersectionObserver and with reduced motion", async ({ page }, testInfo) => {
  const contract = projectContract(testInfo);
...
test("critical public page has no serious automated accessibility violation", async ({ page }, testInfo) => {
  const contract = projectContract(testInfo);
  await openPublicPage(page, contract);
