...
import { canonicalPublisherUrl } from "../../lib/news-growth-authority";
import { providerReadinessSummaryForDomain } from "../../lib/news-provider-readiness";
import {
...
const FEED_CATALOG_CACHE_MS = 5 * 60 * 1_000;

const thumbnailCache = new Map<string, { value: string | null; expiresAt: number }>();
const feedCatalogCache = new Map<string, {
  items: Map<string, NewsThumbnailDiscovery>;
  expiresAt: number;
}>();
const feedCatalogInflight = new Map<string, Promise<Map<string, NewsThumbnailDiscovery>>>();
...
async function loadFeedThumbnailCatalog(
  source: NewsSourceRegistryEntry,
  fetchImpl: typeof fetch,
): Promise<Map<string, NewsThumbnailDiscovery>> {
  ...
  const output = new Map<string, NewsThumbnailDiscovery>();
  for (const block of feedItemBlocks(xml).slice(0, 400)) {
    const articleUrl = feedBlockArticleUrl(block, source);
    if (!articleUrl || output.has(articleUrl)) continue;
    const discovery = extractNewsThumbnailFromFeedBlock(block, articleUrl);
    if (discovery) output.set(articleUrl, discovery);
  }
  return output;
}
...
async function feedThumbnail(
  ...
  if (fetchImpl !== fetch) {
    try {
      return (await loadFeedThumbnailCatalog(source, fetchImpl)).get(articleUrl) ?? null;
    } catch {
      return null;
    }
  }

  const cached = feedCatalogCache.get(source.id);
  if (cached && cached.expiresAt > Date.now()) return cached.items.get(articleUrl) ?? null;

  let inflight = feedCatalogInflight.get(source.id);
  if (!inflight) {
    inflight = loadFeedThumbnailCatalog(source, fetchImpl)
      .catch(() => new Map<string, NewsThumbnailDiscovery>())
      .finally(() => feedCatalogInflight.delete(source.id));
    feedCatalogInflight.set(source.id, inflight);
  }
  const items = await inflight;
  feedCatalogCache.set(source.id, {
    items,
    expiresAt: Date.now() + FEED_CATALOG_CACHE_MS,
  });
  return items.get(articleUrl) ?? null;
}
