...
describe("phone OTP source contract", () => {
  it("keeps local digest verification atomic and separate from one-time DB consumption", async () => {
    ...
    assert.match(authority, /attempt_count = \$2/);
    assert.match(authority, /otp_code_digest/);
    assert.match(authority, /otp_code_digest = NULL/);
    assert.match(authority, /verifyPhoneOtpChallenge/);
    assert.match(authority, /verifyPhoneOtpCode/);
    ...
    assert.doesNotMatch(verifyRoute, /claimPhoneOtpVerification|completePhoneOtpVerification/);
