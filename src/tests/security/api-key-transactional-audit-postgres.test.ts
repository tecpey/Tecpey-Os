...
describe("API key transactional audit authority", () => {
    ...
    async () => {
      ...
      await withClient(async (client) => {
        const result = await client.query<{ count: string }>(
          `SELECT COUNT(*)::text AS count FROM api_keys WHERE user_id = $1`,
          [userId],
        );
  ...
  it(
    ...
    async () => {
      ...
      assert.deepEqual(await keyRow(created.apiKey.id), firstStored);
    },
  );
  ...
  it(
    ...
    async () => {
      ...
      assert.equal(
        await setApiKeyActive(created.apiKey.id, userId, false, auditContext(userId, "disable")),
        true,
      );
      assert.equal((await keyRow(created.apiKey.id))?.is_active, false);

      assert.equal(
        await setApiKeyActive(created.apiKey.id, userId, true, auditContext(userId, "enable")),
        true,
      );
      assert.equal((await keyRow(created.apiKey.id))?.is_active, true);

      const deleteAudit = auditContext(userId, "delete");
      assert.equal(await deleteApiKey(created.apiKey.id, userId, deleteAudit), true);
  ...
  it(
    ...
    async () => {
      ...
      assert.equal(
        await setApiKeyActive(created.apiKey.id, attackerId, false, attackerAudit),
        false,
      );
      assert.equal((await keyRow(created.apiKey.id))?.is_active, true);
