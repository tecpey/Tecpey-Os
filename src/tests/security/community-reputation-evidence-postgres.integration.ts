...
async function seedIdentity(label: string): Promise<Identity> {
  ...
  await withClient(async (client) => {
    ...
    try {
      ...
      // The student insert legitimately creates TecPey's default binding and
      // public-profile FK. Keep it intact, then add the independent tenant
      // binding used by this multi-tenant evidence test.
      await client.query(
...
describe("Community reputation evidence PostgreSQL authority", () => {
  it("materializes exact interactive completion evidence in the same terminal update", {
    ...
    await withClient(async (client) => {
      const replayed = await materializeCommunityReputationEvidenceTx(
        client,
        enrollment,
      );
  ...
  it("materializes worker not-completed evidence with deterministic rounding", {
    skip: !configured,
  }, async () => {
  ...
  it("rejects active sources, conflicting evidence and ledger mutation", {
    skip: !configured,
  }, async () => {
      ...
      withClient((client) => client.query(
        `INSERT INTO academy_community_reputation_evidence
         ...
         SELECT id, tenant_id, workspace_id, principal_type, principal_id, student_id,
                ...
                coverage_basis_points, TRUE, finalization_source, finalization_run_id, $2
           FROM academy_community_challenge_enrollments
          WHERE id = $1::uuid`,
        [terminalId, "f".repeat(64)],
      )),
      /community reputation evidence does not match terminal source|duplicate key/,
    );
  ...
  it("rolls back finalization when the selected tenant binding is revoked", {
    skip: !configured,
  }, async () => {
  ...
  it("returns principal-bound aggregates and hides evidence after binding revocation", {
    skip: !configured,
  }, async () => {
    const alpha = await seedIdentity("alpha");
    const beta = await seedIdentity("beta");
    const alphaCycleOne = await previousCycle(7);
    const alphaCycleTwo = await previousCycle(8);
