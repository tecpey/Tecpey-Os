...
function item(id: string, payload: Record<string, unknown> = { lessonSlug: "lesson-1" }): OfflineSyncItem {
  return {
    id,
...
async function cleanupStudent(client: PoolClient, studentId: string): Promise<void> {
  ...
  await client.query("DELETE FROM academy_learning_brain WHERE student_id = $1::uuid", [studentId]);
...
describe("Offline sync PostgreSQL command authority", () => {
  it(
    ...
    { skip: !databaseConfigured, timeout: 60_000 },
    async () => {
      const client = await pool!.connect();
      const studentId = await createStudent(client, "concurrent");
      client.release();
      ...
      try {
        ...
        assert.equal(results.filter((result) => result.replayed === true).length, 11);
        assert.equal(new Set(results.map((result) => result.learningEventId)).size, 1);

        const verify = await pool!.connect();
        try {
          const commands = await verify.query<{ count: string }>(
            `SELECT COUNT(*)::text AS count
               FROM offline_sync_commands
              WHERE tenant_id = $1 AND student_id = $2::uuid AND client_event_id = $3`,
          ...
          assert.equal(Number(commands.rows[0]?.count), 1);
          assert.equal(Number(events.rows[0]?.count), 1);
        } finally {
          await cleanupStudent(verify, studentId);
          verify.release();
        }
      } catch (error) {
        const cleanup = await pool!.connect();
        try {
          await cleanupStudent(cleanup, studentId);
        } finally {
          cleanup.release();
        }
        throw error;
  ...
  it(
    ...
    async () => {
      const client = await pool!.connect();
      const studentId = await createStudent(client, "conflict");
      client.release();
      const clientEventId = `offline-${randomUUID()}`;
  ...
  it(
    ...
    async () => {
      const client = await pool!.connect();
      const firstStudent = await createStudent(client, "student-a");
      const secondStudent = await createStudent(client, "student-b");
      client.release();
      const clientEventId = `offline-${randomUUID()}`;
      ...
      try {
        ...
        const cleanup = await pool!.connect();
        try {
          await cleanupStudent(cleanup, firstStudent);
          await cleanupStudent(cleanup, secondStudent);
        } finally {
          cleanup.release();
        }
      }
  ...
  it(
    ...
    async () => {
      ...
      const studentId = await createStudent(setup, "rollback");
      ...
      try {
        ...
        const cleanup = await pool!.connect();
        try {
          await cleanup.query(`DROP TRIGGER IF EXISTS ${triggerName} ON learning_events`);
  ...
  it(
    ...
    async () => {
      const client = await pool!.connect();
      const studentId = await createStudent(client, "reconcile");
      ...
      try {
        ...
        for (const [clientEventId, eventId] of [
          ...
          [retryableClientId, offlineLearningEventId({
            tenantId: PLATFORM.DEFAULT_TENANT_ID,
            studentId,
          ...
          await client.query(
            `INSERT INTO offline_sync_commands
              (tenant_id, student_id, client_event_id, command_hash, event_type,
               source, locale, client_created_at, payload, status, domain_event_id,
               processing_started_at, retain_until)
             VALUES ($1, $2::uuid, $3, $4, 'lesson_viewed', 'pwa', 'fa', NOW(),
                     '{}'::jsonb, 'processing', $5, NOW() - INTERVAL '10 minutes',
                     NOW() + INTERVAL '90 days')`,
            [PLATFORM.DEFAULT_TENANT_ID, studentId, clientEventId, "a".repeat(64), eventId],
          );
        }
        await client.query(
          `INSERT INTO offline_sync_commands
            (tenant_id, student_id, client_event_id, command_hash, event_type,
             source, locale, client_created_at, payload, status, last_error_code,
             created_at, updated_at, retain_until)
           VALUES ($1, $2::uuid, $3, $4, 'lesson_viewed', 'pwa', 'fa', NOW(),
                   '{}'::jsonb, 'rejected', 'invalid_event',
                   NOW() - INTERVAL '100 days', NOW() - INTERVAL '100 days',
                   NOW() - INTERVAL '1 day')`,
          [PLATFORM.DEFAULT_TENANT_ID, studentId, expiredClientId, "b".repeat(64)],
        );

        await client.query("BEGIN");
        const reconciled = await reconcileStaleOfflineCommands(client, { limit: 10 });
        const purged = await purgeExpiredOfflineCommands(client, 10);
        await client.query("COMMIT");
        assert.deepEqual(reconciled, { committed: 1, retryable: 1 });
        assert.equal(purged, 1);

        const states = await client.query<{ client_event_id: string; status: string }>(
  ...
  it("returns retryable and never false success when PostgreSQL is unavailable", () => {
    const script = `
      ...
      const processOfflineSyncCommand = module.processOfflineSyncCommand ?? module.default?.processOfflineSyncCommand;
      const result = await processOfflineSyncCommand({
        tenantId: "tecpey",
