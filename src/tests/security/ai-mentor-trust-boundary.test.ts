...
describe("AI Mentor trust boundary", () => {
  ...
  it("does not turn missing learner evidence into neutral profile facts", () => {
    ...
    assert.equal(
      parsed.serverContext.profile.levelEvidenceState,
      "provisional",
    );
    ...
    assert.equal(
      parsed.serverContext.profile.confidenceEvidenceState,
      "provisional",
    );
    assert.equal(parsed.serverContext.profile.disciplineScore, null);
    assert.equal(
      parsed.serverContext.profile.disciplineEvidenceState,
      "unknown",
    );
    assert.equal(parsed.serverContext.profile.learningStyle, null);
    assert.equal(
      parsed.serverContext.profile.learningStyleEvidenceState,
      "unknown",
    );
  });

  it("exposes profile values only after the corresponding evidence threshold", () => {
