...
async function createAuthorization(input: {
  ...
  idempotencyKey: string;
  destinationAddress?: string;
}): Promise<{ authorizationId: string; requestHash: string }> {
...
describe("PostgreSQL withdrawal admission authority", () => {
  it(
    ...
    async () => {
      ...
      try {
        ...
        const first = await createAuthoritativeWithdrawal(input);
        assert.equal(first.ok, true);
        if (!first.ok) return;
        assert.equal(first.replayed, false);
        assert.equal(first.withdrawal.state, "compliance_review");

        const replay = await createAuthoritativeWithdrawal(input);
  ...
  it(
    ...
    async () => {
      ...
      try {
        const first = await createAuthoritativeWithdrawal(
          requestInput({
            userId,
            amount: "1",
            idempotencyKey: key,
        ...
        const changed = await createAuthoritativeWithdrawal(
          requestInput({
            userId,
            amount: "2",
            idempotencyKey: key,
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const second = await createAuthoritativeWithdrawal(
          requestInput({
            userId,
            amount: "1",
        ...
        assert.deepEqual(second, {
          ok: false,
          reason: "withdrawal_authorization_invalid",
          code: 403,
        });
      } finally {
        await cleanup(userId);
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const evidence = await withDb(async (client) => {
          const withdrawal = await client.query<{ count: string }>(
            "SELECT COUNT(*)::text AS count FROM withdrawals WHERE user_id = $1",
            [userId],
          );
          ...
          return {
            withdrawalCount: Number(withdrawal.rows[0]?.count ?? "0"),
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const cancelled = await cancelAuthoritativeWithdrawal(
