...
describe("Academy path progress aggregation", () => {
  it("unlocks terms only from officially passed previous terms", () => {
    const progress = buildAcademyPathProgress(
      ...
      [
        ...
        { locale: "fa", termNumber: 2, termSlug: "term-2", totalSections: 6, completedSections: 3, answeredSections: 1, percent: 50, xp: 35, updatedAt: null },
      ],
    );
    ...
    assert.equal(progress["term-2"].xp, 60);
  ...
  it("shows lesson progress before the official quiz without unlocking the next term", () => {
    const progress = buildAcademyPathProgress(
      [],
      [
        { locale: "en", termNumber: 1, termSlug: "term-1", totalSections: 5, completedSections: 2, answeredSections: 1, percent: 40, xp: 25, updatedAt: null },
      ],
    );

    assert.equal(progress["term-1"].progress, 40);
    assert.equal(progress["term-1"].xp, 25);
