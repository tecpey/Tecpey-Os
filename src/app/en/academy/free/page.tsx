    ...
    <EnglishShell>
      ...
      <div className="px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl space-y-8">
          ...
          <section className="grid gap-4 lg:grid-cols-2"><div className="rounded-[30px] border border-emerald-200 bg-white/92 p-6 shadow-sm dark:border-emerald-300/15 dark:bg-white/[0.055]"><BookOpenCheck className="h-8 w-8 text-emerald-500" aria-hidden="true" /><h2 className="mt-4 text-2xl font-black text-slate-950 dark:text-white">Next learning routes</h2><div className="mt-4 flex flex-wrap gap-2">{internalLinks.map(([title, href]) => <Link key={href} href={href} className="rounded-full border border-cyan-200 bg-white px-4 py-2 text-xs font-black text-cyan-800">{title}</Link>)}</div></div><div className="rounded-[30px] border border-cyan-300/15 bg-white/92 p-6 shadow-sm dark:bg-white/[0.055]"><CheckCircle2 className="h-8 w-8 text-cyan-500" aria-hidden="true" /><h2 className="mt-4 text-2xl font-black text-slate-950 dark:text-white">Market context, not trade pressure</h2><div className="mt-4 flex flex-wrap gap-2">{relationLinks.map(([title, href]) => <Link key={href} href={href} className="rounded-full border border-cyan-200 bg-white px-4 py-2 text-xs font-black text-cyan-800">{title}</Link>)}</div></div></section>
        </div>
      </div>
    </EnglishShell>
  );
