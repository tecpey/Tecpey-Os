...
export default function CryptoPageClient({
  ...
  // initialCoin was prefetched on the server for this symbol — see
  // src/app/crypto/[symbol]/page.tsx. Once the live price hook's own data
  // arrives (or the socket ticker updates it), that takes over; until then
  // this plain fallback avoids a blank header/stats/about section.
  const liveCoin = currencies?.find((c) => c.symbol === symbol) ?? initialCoin;
