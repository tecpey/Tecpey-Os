        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {coinPages.slice(0, 16).map((coin) => (
              <Link key={coin.slug} href={`/price/${coin.slug}`} className="group overflow-hidden rounded-[28px] border border-slate-200 bg-white/85 p-3 transition hover:-translate-y-1 hover:border-cyan-300 hover:shadow-[0_24px_70px_rgba(34,211,238,.14)] focus:outline-none focus:ring-2 focus:ring-cyan-300/60 dark:border-white/10 dark:bg-white/[0.04]">
                <CoinVisual symbol={coin.symbol} slug={coin.slug} name={coin.name} faName={coin.faName} variant="thumb" />
                <div className="pt-4">
