...
export default function MarketsPageClient({
  ...
  const effectiveResult = data ?? (isDefaultView ? initialCurrencies : undefined);

  const pageCurrencies = useMemo(() => {
    return effectiveResult?.data ?? [];
  }, [effectiveResult]);
  ...
  const totalPages = Math.max(1, Number(effectiveResult?.meta?.last_page ?? 1));
