...
import { loadCommunityReputationEvidenceSummary } from "@/lib/community-reputation-evidence-authority";
  ...
  return withObservability(
    ...
    async () => {
      ...
      if ([...url.searchParams.keys()].length > 0) {
        return noStore(apiError("invalid_community_reputation_query", 400));
      }
      ...
      const limited = await rateLimit(req, {
        namespace: "community-reputation-evidence-read",
      ...
      if (!context.available) {
        return noStore(apiError("community_reputation_unavailable", 503));
      }

      const loaded = await loadCommunityReputationEvidenceSummary(context);
      if (!loaded.available) {
        return noStore(apiError("community_reputation_unavailable", 503));
      }
      return noStore(apiOk({ summary: loaded.summary }));
    },
  );
}
