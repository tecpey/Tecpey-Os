...
async function hasPreviousTermPassed(client: Queryable, studentId: string, termNumber: number, locale: string) {
  ...
  const row = await client.query(
    `SELECT 1 FROM academy_term_progress
     WHERE student_id = $1::uuid AND term_number = $2 AND locale = $3 AND status = 'passed'
     LIMIT 1`,
...
export async function POST(req: NextRequest) {
  return withObservability(req, { route: "/api/academy-term-progress" }, async () => {
    ...
    try {
      ...
      if (idempotencyKey && idempotencyKey.length < 8) return apiError("invalid_idempotency_key", 400);
      const commandType = `term_assessment:${locale}:${termNumber}`;
      const commandRequest = { termNumber, locale, answers: submitted, attemptLog };

      const result = await withTx(async (client) => {
        await client.query(
          `SELECT pg_advisory_xact_lock(hashtext('academy_term_assessment'), hashtext($1))`,
        ...
        await client.query(
          ...
          [
            ...
            JSON.stringify({
              ...
              authority: "server_term_assessment_v1",
            }),
            getClientIp(req),
          ],
        );
