...
type Admission =
  ...
  | { ok: false; reason: "budget_exhausted" | "duplicate_attempt" | "authority_unavailable" };
...
async function settleProviderAttempt(input: {
  ...
  await transaction(input.client, async () => {
    const selected = await input.client.query<{
      ...
      `SELECT budget_day, reserved_usd_micros, settled_usd_micros, status
         FROM platform_news_ai_provider_attempts
...
function blockedResponse(reason: Admission extends { ok: false; reason: infer R } ? R : never): Response {
