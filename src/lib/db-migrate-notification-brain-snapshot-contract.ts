...
const FILENAME = "0063_notification_brain_snapshot_contract.sql";

export const NOTIFICATION_BRAIN_SNAPSHOT_CONTRACT_SQL = `
...
export async function runNotificationBrainSnapshotContractMigrations(
  client: PoolClient,
): Promise<void> {
  const cs = checksum(NOTIFICATION_BRAIN_SNAPSHOT_CONTRACT_SQL);
  ...
  if (applied.rows[0]) {
    if (applied.rows[0].checksum !== cs) {
      throw new Error(
        `[db-migrate-notification-brain-snapshot-contract] checksum mismatch for ${FILENAME}`,
      );
    }
    return;
  }

  logger.info("[db-migrate-notification-brain-snapshot-contract] applying migration", {
    filename: FILENAME,
    checksum: cs,
  });
  await client.query("BEGIN");
  try {
    await client.query(NOTIFICATION_BRAIN_SNAPSHOT_CONTRACT_SQL);
    await client.query(
