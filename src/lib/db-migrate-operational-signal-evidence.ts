...
CREATE TABLE IF NOT EXISTS platform_operational_signals (
  ...
  CONSTRAINT platform_operational_signal_detector_check
    CHECK (detector ~ '^[a-z0-9][a-z0-9._:-]{2,119}$'),
  CONSTRAINT platform_operational_signal_severity_check
    CHECK (severity IN ('warning', 'critical')),
  CONSTRAINT platform_operational_signal_status_check
    CHECK (status_classification IN ('active', 'authority_unavailable')),
  CONSTRAINT platform_operational_signal_time_check
    CHECK (dedupe_bucket_at <= occurred_at),
...
CREATE INDEX IF NOT EXISTS platform_operational_signals_lookup_idx
  ON platform_operational_signals (signal_type, occurred_at DESC, severity);
...
export async function runOperationalSignalEvidenceMigrations(
  ...
  CONSTRAINT platform_operational_signal_attempt_semantics_check
    CHECK (
      (
      ...
      OR (
        delivery_result = 'retryable_failure'
      ...
      OR (
        delivery_result = 'terminal_failure'
        AND error_code IS NOT NULL
