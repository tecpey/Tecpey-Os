...
CREATE TABLE IF NOT EXISTS platform_operational_signals (
  ...
  CONSTRAINT platform_operational_signal_id_check
    CHECK (signal_id ~ '^[A-Za-z0-9._:-]{8,240}$'),
  ...
  CONSTRAINT platform_operational_signal_condition_check
    CHECK (condition ~ '^[a-z0-9][a-z0-9._:-]{2,99}$'),
  ...
  CONSTRAINT platform_operational_signal_incident_lifecycle_key
...
CREATE INDEX IF NOT EXISTS platform_operational_signals_lookup_idx
  ON platform_operational_signals
    (source, condition, occurred_at DESC);
...
DROP TRIGGER IF EXISTS platform_operational_signal_attempts_immutable
  ON platform_operational_signal_delivery_attempts;
CREATE TRIGGER platform_operational_signal_attempts_immutable
