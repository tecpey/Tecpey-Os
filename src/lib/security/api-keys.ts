...
export async function validateApiKey(
  ...
  const result = await withDb(async (client) => {
    ...
    if (!key.is_active) return { valid: false, userId: key.user_id, keyId: key.id, permissions, reason: "key_disabled" };
    if (key.expires_at && key.expires_at < new Date()) return { valid: false, userId: key.user_id, keyId: key.id, permissions, reason: "key_expired" };
    if (!permissions.includes(requiredPermission)) return { valid: false, userId: key.user_id, keyId: key.id, permissions, reason: "insufficient_permissions" };
    if (key.ip_whitelist && key.ip_whitelist.length > 0 && callerIp && !ipInWhitelist(callerIp, key.ip_whitelist)) {
      return { valid: false, userId: key.user_id, keyId: key.id, permissions, reason: "ip_not_whitelisted" };
    }
    ...
    return { valid: true, userId: key.user_id, keyId: key.id, permissions };
  });

  if (!result.enabled) return { valid: false, userId: null, keyId: null, permissions: [], reason: "db_unavailable" };
  return result.value;
}
...
export async function rotateApiKey(
  ...
  const result = await withTx(async (client) => {
    ...
    await writeSensitiveMutationAuditTx(client, {
      ...
      resourceId: keyId,
      outcome: "success",
      metadata: { credentialFingerprint: generated.credentialFingerprint },
    });
