...
export function resolveAdminControlPlaneMatrix(input: {
  ...
  const modules: AdminControlPlaneModule[] = [
    ...
    },
    {
      ...
      riskLevel: "sensitive",
      adminRoute: "/command-center",
      apiRoutes: ["/api/command-center/campaign", "/api/notifications/*"],
      requiredPermission: "admin.roles.manage",
      stepUpRequired: true,
      gatedBy: [],
      connections: [
        { id: "notification_outbox", labelFa: "Notification Outbox", labelEn: "Notification outbox", kind: "messaging", status: "internal" },
        { id: "transactional_delivery", labelFa: "Transactional Delivery", labelEn: "Transactional delivery", kind: "messaging", status: "planned", lockedReasonFa: "اتصال provider بیرونی باید Secret، bounce policy و audit داشته باشد.", lockedReasonEn: "External provider connection needs secrets, bounce policy and audit." },
      ],
      controls: [
