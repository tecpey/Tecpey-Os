    ...
    <section className="relative isolate overflow-hidden px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
      ...
      <div className="mx-auto max-w-7xl">
        <div className="max-w-4xl">
          ...
          <p className="mt-5 max-w-3xl text-pretty text-base leading-8 text-[color:var(--tp-muted)] sm:text-lg">
            {description}
          </p>
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <Link href={ctaHref} className="tecpey-action-primary">
              {ctaLabel}
              <ArrowLeft className="h-5 w-5" />
            </Link>
            <Link href={secondaryHref} className="tecpey-action-secondary">
              {secondaryLabel}
              <Search className="h-5 w-5 text-[color:var(--tp-primary)]" />
            </Link>
          </div>
        </div>
      </div>
  ...
  return (
    <section className="px-4 pb-10 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-3">
        {items.map((item) => (
          <div key={item.title} className="tecpey-card p-5">
            <item.icon className="h-7 w-7 text-[color:var(--tp-primary)]" />
            <h3 className="mt-4 text-lg font-black">{item.title}</h3>
            <p className="mt-2 text-sm leading-7 text-[color:var(--tp-muted)]">{item.desc}</p>
          </div>
        ))}
      </div>
    </section>
...
export function ArticleCard({
  ...
  return (
    <Link href={href} className="tecpey-card tecpey-pressable group block h-full p-6">
      {meta && <p className="text-xs font-black text-[color:var(--tp-primary)]">{meta}</p>}
      <h3 className="mt-3 text-xl font-black leading-8 text-slate-950 dark:text-white">{title}</h3>
      <p className="mt-3 text-sm leading-7 text-[color:var(--tp-muted)]">{description}</p>
      <div className="mt-5 inline-flex items-center gap-2 text-sm font-black text-[color:var(--tp-primary)]">
...
export function FaqList({ faqs }: { faqs: { q: string; a: string }[] }) {
  return (
    <div className="space-y-3">
      {faqs.map((item) => (
        <div key={item.q} className="tecpey-card p-5">
          <h3 className="flex items-start gap-2 text-base font-black">
            <CheckCircle2 className="mt-1 h-5 w-5 shrink-0 text-emerald-500" />
            {item.q}
          </h3>
          <p className="mt-3 text-sm leading-8 text-[color:var(--tp-muted)]">{item.a}</p>
        </div>
      ))}
    </div>
  );
...
export function SeoNote() {
  return (
    <div className="rounded-[32px] border border-[color:var(--tp-border)] bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,.18),transparent_35%),linear-gradient(145deg,#07111f,#0f172a)] p-6 text-white shadow-xl shadow-cyan-500/10">
