  ...
  return <main className={styles.story} data-runtime-contract="landing-growth-v1" lang={fa ? "fa" : "en"} dir={fa ? "rtl" : "ltr"}>
    ...
    <Chapter id="story-academy" stage={3} locale={locale} labelledBy="story-academy-title">
      <div className={styles.academyIntro}>
        ...
        <aside className={styles.mentorCard}><PremiumIcon icon={BrainCircuit} size="md" /><div><strong>{t("منتور در تمام مسیر", "A mentor throughout")}</strong><p>{t("برای فهم درس، مرور اشتباه و انتخاب تمرین بعدی؛ منتور همراه یادگیری است و سیگنال خرید یا فروش نمی‌فروشد.", "For lesson clarity, mistake review and the next practice choice. The mentor supports learning; it does not sell buy or sell signals.")}</p></div><Link href={link("/academy/ai-guide")} aria-label={t("آشنایی با منتور تک‌پی", "Meet TecPey Mentor")}><ArrowUpRight size={20} /></Link></aside>
      </div>
    ...
    <Chapter id="story-practice" stage={4} locale={locale} tone="dark" labelledBy="story-practice-title">
      <SectionHeader icon={LineChart} kicker={t("تمرین با سرمایه مجازی", "Practice with virtual capital")} id="story-practice-title" title={t("دانش را به تصمیم قابل بازبینی تبدیل کن", "Turn knowledge into reviewable decisions")} description={t("در تریدینگ آرنا اجرای معامله و کنترل ریسک را با سرمایه مجازی تمرین کن؛ بدون پول واقعی. سپس دلیل، احساس و نتیجه هر تصمیم را در ژورنال نگه دار و با منتور مرور کن.", "Practice execution and risk control with virtual capital in the Trading Arena: no real money involved. Then keep the reason, emotion and outcome of each decision in your journal and review it with a mentor.")} />
      <div className={styles.practiceScene}>
        ...
        <div className={styles.practiceSteps}><div><LineChart size={22} /><span><strong>{t("تریدینگ آرنا", "Trading Arena")}</strong><small>{t("اجرای سناریو با سرمایه مجازی", "Execute a virtual-capital scenario")}</small></span></div><div><NotebookPen size={22} /><span><strong>{t("ژورنال تصمیم", "Decision journal")}</strong><small>{t("این تمرین پول، سود یا معامله واقعی ندارد.", "The boundary is explicit: no real money, real profit or real trade takes place in it.")}</small></span></div><div><BrainCircuit size={22} /><span><strong>{t("تحلیل با منتور", "Mentor analysis")}</strong><small>{t("کشف الگو، نه وعده سود", "Find patterns, never profit promises")}</small></span></div><Link className={styles.primary} href={link("/academy/trading-arena")}>{t("شروع تمرین", "Start practicing")}<ArrowUpRight size={18} /></Link></div>
      </div>
    </Chapter>
