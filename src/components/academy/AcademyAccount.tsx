...
export function AcademyAccount({ locale }: { locale: "fa" | "en" }) {
  ...
  return <main dir={isFa ? "rtl" : "ltr"} className="min-h-[70vh] bg-bg px-4 py-10 text-fg sm:px-6">
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mt-6 divide-y divide-fg/10">{rows.map(({href,Icon,title,text}) => <Link key={href} href={href} className="flex min-h-20 items-center gap-4 rounded-xl px-3 py-4 transition-colors motion-reduce:transition-none hover:bg-fg/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"><Icon className="h-5 w-5 shrink-0 text-cyan-600 dark:text-cyan-200" aria-hidden="true"/><div><h2 className="font-semibold">{title}</h2><p className="mt-1 text-sm leading-6 text-muted">{text}</p></div></Link>)}</div>
