...
function ReflectionEditor({
  ...
  return (
    <section
      ...
      aria-labelledby={`reflection-${trade.id}`}
      aria-busy={saving}
    >
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <label className="block text-xs font-black text-slate-400">
          ...
          <textarea
            ...
            maxLength={4_000}
            rows={4}
            className="mt-2 w-full resize-y rounded-2xl border border-white/10 bg-slate-950/50 p-3 text-sm font-bold leading-7 text-white outline-none transition focus:border-violet-300/50 disabled:cursor-wait disabled:opacity-60"
      ...
      <fieldset disabled={saving} className="mt-4 disabled:cursor-wait disabled:opacity-60">
