...
Within each configurable dedupe window (default one hour), the **first observation** for the same detector instance, service, component, severity and reason-code set becomes the durable signal. Detector instance identity is represented only by a 24-hex privacy-safe fingerprint derived from the host and its operational state root; the raw hostname is never placed in the signal payload. Two hosts therefore cannot collapse simultaneous failures into one webhook idempotency key. Later observations from the same instance in that window replay the same signal identity instead of creating an alert storm. The measurements in that signal are intentionally the **first firing snapshot for the window**, not live telemetry. A later window creates a new reminder identity if the critical condition still exists.
...
`TECPEY_OPS_STATE_DIR` is part of detector-instance identity and **must be deployment-scoped**. Do not share one operational state root between unrelated staging/production deployments or between independently managed TecPey instances. The installer creates the root with mode `0700`; changing or reusing the root is an operational identity change and requires a reviewed migration/activation procedure.
