...
CSP is applied via `proxy.ts` (nonce-based, per-request). Browser connection
authority is generated by the typed fail-closed policy in
`src/lib/security/csp-connection-policy.ts`: production requires exact HTTPS and
WSS origins and never falls back to broad `https:`, `ws:` or `wss:` sources.
The governed Browser Golden Path observes privacy-minimized CSP violation
events. The owned-origin registry, reporting decision and operating contract
are documented in `docs/security/CSP_CONNECTION_POLICY.md`.
