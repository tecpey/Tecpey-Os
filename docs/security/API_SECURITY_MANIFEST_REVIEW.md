...
- [x] every local and delegated handler that consumes a request body is evaluated as a separate allocation boundary;
- [x] a bounded canonical handler cannot hide an unbounded compatibility alias that parses the body first;
...
- [x] governed bounded readers are defined once and shared by parser and authority detection;
...
- [x] negative tests cover stale totals, forged findings, unresolved evidence, import/comment-only evidence, delegated body-boundary bypasses, blanket overrides, unsafe caching, wildcard exceptions, and unbounded bodies;
