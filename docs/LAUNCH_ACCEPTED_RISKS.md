...
> **Reconciliation update (2026-08-01).** Substantially reduced. Canonical Academy progress, assessments, and certificates are now PostgreSQL-authoritative (`src/lib/academy-progress.ts`, `src/lib/academy-assessment.ts`, `src/lib/academy-certificates.ts`, with `academy-progress-*-postgres` authority tests), so the "irreversible loss on clearing browser data / switching devices" premise no longer applies to canonical state. Residual browser-local state is limited to disposable presentation and quarantined legacy simulation modules, inventoried by `scripts/check-browser-persistence.mjs`. See `docs/PRODUCTION_DECISIONS.md` D-02.
