...
| Accepted risks | Every remaining non-blocking risk has a named owner, expiration/review date, mitigation, rollback condition and approval owner; the final owner sign-off artifact passes `scripts/verify-accepted-risk-signoff-evidence.mjs`. | Accepted for NOG-08 on exact runtime candidate `79c48a16cb685a88315a44e103b3758cf7845d65`: three attributable owners cover all nine controlled-launch risks, the register digest and current review dates are recorded, immutable issue-comment IDs are bound to each risk, and CI verifies live GitHub authorship plus exact body digests. Go remains blocked by NOG-09. |
