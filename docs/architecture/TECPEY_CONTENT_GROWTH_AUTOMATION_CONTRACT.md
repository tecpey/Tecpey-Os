...
`tecpey-organic-growth-policy-v2`. Automated coin, tool and news publication must
...
## Organic Growth OS — 2026 Operating Model

TecPey treats organic growth as a governed operating system, not as a keyword
campaign. The same evidence used to decide whether a News, Coin or Tool entity is
useful to a learner must also power search discovery, answer-engine extraction,
generative-engine citation and internal navigation.

The code authority is `tecpey-organic-growth-policy-v2`. A page generated by an
automation is not growth-ready unless all three readiness dimensions pass:

1. **SEO readiness** — unique canonical page, sufficient localized metadata,
   crawlable internal links, entity-rich keywords/tags, source attribution and
   visible structured-data parity;
2. **AEO readiness** — direct answer summary, real question intents and factual
   key-facts that answer what happened, why it matters and what the learner
   should verify without inventing FAQ/schema spam;
3. **GEO readiness** — concise citation-ready summary, explicit publisher and
   TecPey attribution, canonical entities, freshness and a clear boundary among
   publisher fact, TecPey educational context and AI-assisted translation.

The v2 profile records per-dimension scores and blockers. `ready=false` sends an
automated item to review rather than allowing a thin page to enter the index.
No page receives a higher score merely for repeating a keyword more often.

### News Growth Packet

Every archived news revision can create a growth packet containing:

- original publisher title, lead and publisher-provided feed body;
- canonical publisher URL with tracking parameters removed;
- immutable content hash and Tehran calendar day;
- real Persian translation status and translation evidence;
- coin, tool and topic taxonomy over the entire available text;
- source, entity and question/search-intent coverage;
- canonical TecPey detail URL, internal relations and NewsArticle metadata;
- AEO answer and key-fact candidates;
- GEO citation/LLM summary with the source boundary preserved;
- sitemap/news-sitemap eligibility and optional IndexNow notification;
- publication decision, reasons and organic-growth policy version.

Opening `/crypto-news`, `/coins` or `/trading-tools` must never start ingestion,
translation or external research. These pages are read surfaces. Workers own all
network effects and persistence.

### Daily Archive Authority

`platform_news_archive_items` and `platform_news_archive_translations` are
append-only. The public news center selects one `Asia/Tehran` calendar day at a
time and displays every accepted item for that day (bounded only by the database
safety ceiling). A new day never silently backfills yesterday's cards as today's
news. Historical dates remain addressable and can be filtered by text or by the
coin/tool/topic taxonomy.

Translation is fail-closed. If a translation provider is unavailable or the
translated output fails Persian, numeric-integrity or no-added-advice checks, the
original publisher text remains visible and is explicitly labelled as such. A
generic TecPey summary must never masquerade as a Persian translation.

Completed translations are reused by canonical article URL + content hash. A
failed translation uses a governed retry cooldown rather than spending model
budget every ten minutes on identical input.

## Trend Intelligence and Community Discovery

Trend selection is evidence-driven and resistant to single-community hype.
`tecpey-organic-trend-policy-v1` collects independent signals for Coins, Tools
and Topics and materializes 24-hour, 7-day and 30-day views.

Source families are deliberately separated:

- `market` — ranked market/trending evidence such as CoinGecko;
- `news` — approved publisher feeds and their entity impact;
- `social` — public community evidence such as cited X conversations;
- `search` — cited public-research/search evidence;
- `web` — other cited high-authority public web evidence;
- `official` — first-party project/tool announcements where applicable;
- `editorial` — bounded human TecPey weighting, never a substitute for evidence.

Signals store magnitude, velocity, confidence, source authority, freshness and
manipulation risk. A single viral social source is explicitly penalized. The
`breakout` tier requires cross-family confirmation and at least three independent
sources; otherwise the item remains sustained/emerging/watch regardless of hype.

The public Trend Radar is shared across News, Coins and Tools and exposes three
time windows: **24h / 7d / 30d**. It is a discovery surface, not a trading signal.
Cards link users into TecPey's canonical coin/tool/news knowledge graph instead
of sending traffic directly out of the platform.

### Governed Growth Hacker

The AI control plane includes a `growth_hacker` persona with public-research and
aggregate-metrics scope. It can use governed X/search/web research through xAI,
Perplexity, OpenAI/OpenRouter routes allowed by policy. It cannot:

- read private user data;
- publish directly;
- execute financial actions or generate buy/sell signals;
- create doorway pages, cloaking or keyword stuffing;
- fabricate citations or mark its own knowledge as verified.

Every public trend URL returned by the Growth Hacker must also be present in the
provider's citation evidence. Uncited trend claims are discarded before they can
reach `platform_growth_trend_signals`. The `organic_growth_acceleration` workflow
continues through content review, risk/compliance review and human publish review.

### Growth Feedback Loop

The measurement contract intentionally separates generation from optimization.
Future first-party integrations for Google Analytics 4, Google Search Console and
Bing Webmaster/AI citation reporting should feed privacy-safe aggregate metrics
back into Growth OS. They may influence opportunity prioritization, but they do
not bypass content-quality or editorial gates.
...
public evidence
  -> trend intelligence
  -> entity/content opportunity
  -> SEO + AEO + GEO packet
  -> quality/safety/human gate
  -> canonical publication
  -> sitemap/news sitemap/IndexNow
  -> Search/AI/referral measurement
  -> opportunity review
  -> next evidence-backed improvement
```

The optimization objective is sustainable branded and non-branded discovery,
not raw page count. The Growth C-level view should therefore track quality-indexed
pages, non-branded query coverage, qualified organic sessions, citation/referral
coverage, assisted Academy entry, returning learners and the percentage of trend
opportunities rejected for insufficient evidence.
