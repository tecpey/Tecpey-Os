...
- Landing discovery publishes a typed evidence envelope with source authority, coin/tool counts, high-priority news count and `ready/degraded` status so QA can catch stale or incomplete ranking materialization before the UI claims a complete top-ten surface.
