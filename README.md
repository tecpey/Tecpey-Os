...
The most recent world-class QA analysis is dated **2026-08-06** at exact `main` SHA **`a8c3b34e41c0fc869c51a937fa51b292375ee25c`**. Read the full evidence-based report (Persian) in [`docs/audits/QA_REPORT_2026-08-06.fa.md`](./docs/audits/QA_REPORT_2026-08-06.fa.md); the earlier baseline audit is [`docs/audits/TECPEY_PROJECT_STATE_AUDIT_2026-07-26.md`](./docs/audits/TECPEY_PROJECT_STATE_AUDIT_2026-07-26.md).

At this SHA the repository is ~870 TypeScript/TSX files (~145k lines) across 96 API routes, 165 pages and 270 library modules, guarded by 185 test files, 40 checksum-pinned database migrations and 11 CI workflows. Engineering discipline is high: **0** `@ts-ignore`, a single `: any`, and 5 files carrying TODO/FIXME markers. The `main` checks for quality, repository hygiene, API and sensitive-mutation authority, public browser Golden Path (Chromium + Firefox × FA/EN), container/SBOM/vulnerability enforcement, rollback/volume restore and image provenance complete successfully.

**Recently merged controlled-launch work:** the multi-tenant request-resolution foundation (`resolveRequestTenant`, Host→tenant hint, `platform_tenant_domains`); a HIGH transitive CVE fix (socket.io-parser); cross-tenant isolation proofs advancing the `#109` registry to **14 of 28** tables proven; a repair of never-migrated academy learning-brain columns (migration `0055`); a dedicated public Trading Arena section (FA/EN); and a bilingual, accessible, safe Trader Toolbox detail surface.

**Soft-launch readiness (single-tenant education + virtual Arena, real-money custody OFF): ≈ 85%.** The repository is **not fully production-ready for the complete TecPey vision**. The immediate critical path is: (1) an academy achievement-subsystem schema drift (`achievement_catalog`/`student_achievements` columns the code writes were never migrated — a fresh-deploy fix); (2) production-like backup/restore/recovery evidence (#110) and real staging evidence (#229), which need operational access; then product-content depth for News/Coins/Toolbox (#83). Real-money Exchange and custody remain fail-closed and disabled pending an independent HSM/MPC certification path (#106).
